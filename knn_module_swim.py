"""
KNN MODULE FOR SWIMMING APP
===========================================
This module provides KNN functionality for swimming stroke recognition.
It uses ONLY 8 motion features for swimming stroke classification.
"""
...
class KNNModelSwim:
    """
    K-Nearest Neighbors classifier for swimming stroke recognition.
    Uses only 8 motion features for prediction.
    """

    def __init__(self, n_neighbors=5):
        """
        Initialize KNN model for swimming.
        ...
        # ONLY these 8 motion features for KNN (swimming)
        self.feature_names = [
            "acc_mean",
            "acc_rms", 
            "acc_std",
    ...
    def select_features(self, features_df):
        """
        Select ONLY the 8 motion features for KNN.

        Parameters:
        -----------
        features_df : pandas DataFrame
            Complete features dataframe with all 16 features

        Returns:
        --------
        X_knn : pandas DataFrame
            Features with only 8 motion columns
        y : pandas Series
            Target labels
        """
        ...
        if missing_features:
            # If missing, check if we have alternate names
            alt_names = {
                "acc_mean": "mean_magnitude",
                "acc_rms": "rms_magnitude", 
                "acc_std": "std_magnitude",
                "acc_p2p": "p2p_magnitude",
                "gyro_mean": "gyro_mean_magnitude",
                "gyro_rms": "gyro_rms_magnitude",
                "gyro_std": "gyro_std_magnitude",
                "gyro_p2p": "gyro_p2p_magnitude"
            }
            ...
            for feature in self.feature_names:
                ...
                elif feature in alt_names and alt_names[feature] in features_df.columns:
                    available_features.append(alt_names[feature])
            
            if len(available_features) < 4:  # Need at least 4 features
                raise ValueError(f"Missing required features for KNN. Found: {available_features}")
            
            # Use available features
            X_knn = features_df[available_features].copy()
            self.feature_names = available_features
        else:
            # Use all 8 motion features
    ...
    def cross_validate(self, X_train_scaled, y_train, cv=5):
        """
        Perform cross-validation for more reliable performance estimate.
        IMPORTANT: Uses ONLY training data to prevent data leakage.
        FIXED: Now takes pre-scaled training data.

        Parameters:
        -----------
        X_train_scaled : numpy array
            Already scaled training features
        y_train : numpy array
            Training labels
        cv : int
            Number of folds
        ...
        """
        # Perform cross-validation on ALREADY SCALED training data
...
def create_knn_performance_plots(metrics, data_info):
    ...
    fig_class_acc = go.Figure(data=[
        go.Bar(
            ...
            marker_color=['blue', 'green', 'orange', 'red'][:len(per_class_acc)],
    ...
    fig_class_acc.update_layout(
        title='KNN Accuracy per Stroke Class',
...
# ==========================
def run_knn_pipeline_swim(features_df, test_size=0.2, n_neighbors=5, random_state=42):
    """
    Complete KNN pipeline for swimming - to be called from swim.py.
    FIXED: Cross-validation uses ONLY training data to prevent data leakage.

    Parameters:
    -----------
    features_df : pandas DataFrame
        Features dataframe from main app (with all 16 features)
    test_size : float
    ...
    """
    print("🏊‍♂️ Starting KNN pipeline for swimming...")

    # Initialize KNN model
    knn = KNNModelSwim(n_neighbors=n_neighbors)

    # Step 1: Select only motion features for KNN
    print("📊 Step 1: Selecting motion features for KNN...")
    ...
    cv_results = knn.cross_validate(
        data_splits['X_train_scaled'],  # FIXED: Use already scaled training data
        data_splits['y_train'],         # Training labels
        cv=5
    )
    ...
    print("✅ KNN pipeline for swimming completed successfully!")
    return results
...
# ==========================
def test_knn_module_swim():
    """Quick test to verify the swimming KNN module works."""
    print("🧪 Testing KNN module for swimming...")

    # Create dummy swimming data
    np.random.seed(42)
    n_samples = 100

    # Create dummy swimming features
    dummy_data = {
        # Motion features
        ...
        # Shape features (not used by KNN but present in dataframe)
        ...
        "activity": np.random.choice(['butterfly', 'backstroke', 'breaststroke', 'freestyle'], n_samples)
    }

    features_df = pd.DataFrame(dummy_data)

    # Run KNN pipeline
    results = run_knn_pipeline_swim(features_df, n_neighbors=3)
    ...
    print(f"🎯 Features used: {results['model'].feature_names}")
...
if __name__ == "__main__":
    ...
    test_results = test_knn_module_swim()
    print("✅ KNN module for swimming test passed!")
